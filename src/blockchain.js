...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            const data = `${address}:${new Date().getTime().toString().slice(0, -3)}:starRegistry`;
            resolve(data);
        });
    }
    ...
    getStarsByWalletAddress(address) {
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 1; i < self.chain.length; i++) {
                const bodyBlock = await self.chain[i].getBData();
                if (bodyBlock.data.address === address)
                    stars.push({ owner: address, star: bodyBlock.data.star });
            }
            resolve(stars);
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 1; i < self.chain.length; i++) {
                ...
                if (!currentBlock.validate() || currentBlock.previousBlockHash !== previousBlock.hash) {
